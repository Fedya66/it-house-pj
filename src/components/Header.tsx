...
export const Header: React.FC = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/95 dark:bg-black/80 backdrop-blur-sm shadow-lg' : 'bg-transparent'}`}>
      <div className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2 cursor-pointer" onClick={scrollToTop}>
            ...
            <span className="text-xl font-bold text-black dark:text-white tracking-tight">IT HOUSE</span>
          </div>
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {menuItems.map((item) => (
              <button
                ...
                className="text-black/70 dark:text-white/70 hover:text-red-500 transition-colors duration-200 font-medium"
              >
                {item.label}
              </button>
            ))}
